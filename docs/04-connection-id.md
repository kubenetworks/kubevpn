...
| **Disconnect** | Three modes: by connectionID (exact match), by kubeconfig (resolves to connectionID via K8s API), or all. After removal, `resetCurrentConnection()` picks first remaining. |
