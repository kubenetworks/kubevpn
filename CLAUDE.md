...
| Role | traffic manager 创建/升级, IP 分配, proxy inject, health check, OwnerID | TUN, port-forward, DNS, routes, CIDR 探测 |
| Persisted | ✅ OffloadToConfig | ❌ |

**Connect 流程（控制面 → 数据面）:**
```
User Daemon: CreateOutboundPod → UpgradeDeploy → RentIP → cli.Connect(ipCtx)
Root Daemon: GetIPFromContext → DoConnect (getCIDR → NetworkManager.Start)
```
...
- Generated by `GetConnectionID()` which reads namespace UID directly via K8s API
