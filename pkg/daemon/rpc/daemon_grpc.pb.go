...
type TunConfigServiceClient interface {
	...
	// WatchNamespaceRoutes streams pod route prefixes and service records for a
	// namespace, discovered server-side by the traffic manager (a single shared
	// informer per namespace) instead of a per-client cluster-wide list-watch.
	// The first frame is a full snapshot (Snapshot=true); subsequent frames are
...
func (c *tunConfigServiceClient) WatchNamespaceRoutes(ctx context.Context, in *NamespaceRoutesRequest, opts ...grpc.CallOption) (TunConfigService_WatchNamespaceRoutesClient, error) {
	stream, err := c.cc.NewStream(ctx, &TunConfigService_ServiceDesc.Streams[1], TunConfigService_WatchNamespaceRoutes_FullMethodName, opts...)
	if err != nil {
		return nil, err
	}
	x := &tunConfigServiceWatchNamespaceRoutesClient{stream}
	if err := x.ClientStream.SendMsg(in); err != nil {
		return nil, err
	}
	if err := x.ClientStream.CloseSend(); err != nil {
...
type TunConfigService_WatchNamespaceRoutesClient interface {
	Recv() (*NamespaceRoutesResponse, error)
	grpc.ClientStream
}

type tunConfigServiceWatchNamespaceRoutesClient struct {
	grpc.ClientStream
}

func (x *tunConfigServiceWatchNamespaceRoutesClient) Recv() (*NamespaceRoutesResponse, error) {
	m := new(NamespaceRoutesResponse)
	if err := x.ClientStream.RecvMsg(m); err != nil {
		return nil, err
	}
	return m, nil
...
func _TunConfigService_WatchNamespaceRoutes_Handler(srv interface{}, stream grpc.ServerStream) error {
	m := new(NamespaceRoutesRequest)
	if err := stream.RecvMsg(m); err != nil {
		return err
	}
	return srv.(TunConfigServiceServer).WatchNamespaceRoutes(m, &tunConfigServiceWatchNamespaceRoutesServer{stream})
}

type TunConfigService_WatchNamespaceRoutesServer interface {
	Send(*NamespaceRoutesResponse) error
	grpc.ServerStream
}

type tunConfigServiceWatchNamespaceRoutesServer struct {
	grpc.ServerStream
}

func (x *tunConfigServiceWatchNamespaceRoutesServer) Send(m *NamespaceRoutesResponse) error {
	return x.ServerStream.SendMsg(m)
}
